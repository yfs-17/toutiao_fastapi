...
class Category(Base):
    ...
    name: Mapped[str] = mapped_column(String(50),unique=True,nullable=False,comment="分类名字")
    sort_order: Mapped[int] = mapped_column(default=0,nullable=False,comment="排序")

    def __repr__(self):
        return f"<Category(id={self.id},name={self.name},soft_order={self.sort_order}) %r>"
